...
func TestMap_OmitEmpty(t *testing.T) {
	type A struct {
		Name  string
		Value string    `column:",omitempty"`
		Time  time.Time `column:",omitempty"`
	}
	a := A{}
...
func TestMap_OmitNested(t *testing.T) {
	type A struct {
		Name  string
		Value string
		Time  time.Time `column:",omitnested"`
	}
	a := A{Time: time.Now()}
...
func TestMap_NestedSliceWithStructValues(t *testing.T) {
	type address struct {
		Country string `column:"customCountryName"`
	}

	type person struct {
		Name      string    `column:"name"`
		Addresses []address `column:"addresses"`
	}
...
func TestMap_InterfaceTypeWithMapValue(t *testing.T) {
	type A struct {
		...
		Payload interface{} `column:"payload"`
	}
